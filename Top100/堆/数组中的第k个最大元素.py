        ...
        def partition(nums,k):
            ...
            for num in nums:
                if num>pivot:
                    ...
                    equal.append(num)
            if k<=len(big): #target在big中
                return partition(big,k)
            if len(nums)-len(small)<k:
                return partition(small,k-len(nums)+len(small))
            return pivot
...
class Solution:
    def findKthLargest(self, nums: List[int], k: int) -> int:
        min_value, max_value = min(nums), max(nums)  # 时间复杂度O(n)
        count = [0] * (max_value - min_value + 1)  # 归一化，适合数据范围的桶

        for num in nums:
            count[num - min_value] += 1  # 放到指定桶内

        for i in range(len(count) - 1, -1, -1):  # 从大到小遍历
            k -= count[i]
            if k <= 0:
                return i + min_value
